    ...
    def __init__(self, vol_bnds, voxel_size, use_gpu=True):
        ...
        __global__ void integrate(float * tsdf_vol,
          ...
          color_vol[voxel_idx] = new_b*256*256+new_g*256+new_r;
            ...
            grid_dim_x = min(
                gpu_dev.MAX_GRID_DIM_X, int(
                    np.floor(
                        np.cbrt(n_blocks))))
            grid_dim_y = min(
                gpu_dev.MAX_GRID_DIM_Y, int(
                    np.floor(
                        np.sqrt(
                            n_blocks / grid_dim_x))))
...
def get_view_frustum(depth_im, cam_intr, cam_pose):
    ...
    Retruns np.array shape (3, 5)
    """
    im_h = depth_im.shape[0]
    im_w = depth_im.shape[1]
    max_depth = np.max(depth_im)
    view_frust_pts = np.array(
        [(np.array([0, 0, 0, im_w, im_w]) -
          cam_intr[0, 2]) *
         np.array([0, max_depth, max_depth, max_depth, max_depth]) /
         cam_intr[0, 0], (np.array([0, 0, im_h, 0, im_h]) -
                          cam_intr[1, 2]) *
         np.array([0, max_depth, max_depth, max_depth, max_depth]) /
         cam_intr[1, 1], np.array([0, max_depth, max_depth, max_depth, max_depth])])
